...
export default function BeforeAfterSlider({
  ...
  useEffect(()=>{
    ...
    const move = (x:number)=>{
      ...
      const clamped = Math.min(Math.max((x - r.left) / r.width, 0), 1)
      setPos(clamped)
    }
  ...
  return (
    <div ref={ref} style={{position:'relative', height, width:'100%', borderRadius:16, overflow:'hidden', background:'#111'}}>
      {beforeUrl ? <img src={beforeUrl} alt="" style={{position:'absolute', inset:0, width:'100%', height:'100%', objectFit:'contain'}}/> :
        <div style={{position:'absolute', inset:0, display:'grid', placeItems:'center', color:'rgba(255,255,255,.6)'}}>Waiting for image…</div>}
      {afterUrl && (
